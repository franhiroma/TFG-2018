...
public class Rendija_plus : MonoBehaviour
{
    ...
    private void Update()
    {
        if (activarPuedes == true)
        {
            ...
            && (rendija_1.GetComponent<Transform>().transform.position.y > -340.0f)
            && (rendija_2.GetComponent<Transform>().transform.position.y < 340.0f)
            && (rendija_intermedia.GetComponent<Transform>().transform.localScale.y < 0.56f))
            {

...
public class Anchura_minus : MonoBehaviour
{
    ...
    private void Update()
    {
        if (activarPuedes == true && (SceneManager.GetActiveScene().name == "Rendija_lvl0"))
        {
            if (Input.GetKeyUp(KeyCode.Space)
                && panel_arriba.GetComponent<Transform>().transform.localScale.x > 0.6f
                && panel_abajo.GetComponent<Transform>().transform.localScale.x > 0.6F)
            {
